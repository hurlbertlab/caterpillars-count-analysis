...
gfit = fitG(prlepl15.weekly$week, prlepl15.weekly$meanDensity, weighted.mean(prlepl15.weekly$week, prlepl15.weekly$meanDensity),
            5, 3000, control = list(maxit = 10000), method="L-BFGS-B", lower=c(0,0,0,0,0,0))
# "The warning messages produced are probably due to the sd parameter going negative." -Stack Overflow
...
gfit2 = fitG(prlepl15.daily$julianday, prlepl15.daily$meanDensity, weighted.mean(prlepl15.daily$julianday, prlepl15.daily$meanDensity),
             5, 300, control = list(maxit = 10000), method="L-BFGS-B", lower=c(0,0,0,0,0,0))

# Curve parameters
p2 = gfit2$par
r2.2 = cor(prlepl15.daily$julianday, p[3]*dnorm(prlepl15.daily$julianday, p[1], p[2]))^2
totalDensity = sum(prlepl15.daily$meanDensity)
lines(130:210, p[3]*dnorm(130:210, p[1], p[2]), col = 'blue') # make sure it appears on the right plot
...
# Fit a normal curve using least squares
gfit2 = fitG(prmult15.weekly$julianday, prmult15.weekly$meanDensity, weighted.mean(prmult15.weekly$week, prmult15.weekly$meanDensity),
             5, 3000, control = list(maxit = 10000), method="L-BFGS-B", lower=c(0,0,0,0,0,0))

# Curve parameters
p2 = gfit2$par
r2.2 = cor(prmult15.weekly$week, p[3]*dnorm(prmult15.weekly$week, p[1], p[2]))^2
