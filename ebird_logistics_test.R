...
# TO MOLLY :)
sampling_pr <- read.table('c:/git/caterpillars-count-analysis/sampling_pr.csv', header = T, sep = ',') # might have to change this path
obs_pr <- read.table('c:/git/caterpillars-count-analysis/obs_pr.csv', header = T, sep = ',')
# Molly: I don't know where those first columns came from when reading the data in but I don't think that 
# will mess anything up. I checked it on my computer and it was fine. There are warnings that appear at 
# the end of the for loops that I haven't figured out yet but all the plots come up anyway. Also this script 
# calculates inflection point if you need that, I have it set up to spit out a dataframe of inflection points.

library(bbmle) ##has function 'mle2' which is a wrapper for the base function 'optim'
#xmids<-seq(80,180,20)
#long=as.data.frame(matrix(seq(-100,-66,2), ncol=1, byrow=T))    #x
#lat=as.data.frame(matrix(seq(24,50,2), ncol=1, byrow=T))      #y
...
inflection <- unique(inflection.pt.output) 
inflection <- data.frame(inflection) # warning message is ok?
names(inflection) <- c('scientific_name', 'year', 'inflection_pt')
